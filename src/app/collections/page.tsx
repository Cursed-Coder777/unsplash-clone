...
export default function CollectionsPage() {
    ...
    useEffect(() => {
        // ✅ Only check after session is resolved
        if (status === 'unauthenticated') {
            setLoading(false);
            toast.error("Please login to view your collections");
    ...
    if (status === 'unauthenticated') {
        return (
            <div className=" mt-30 flex flex-col items-center justify-center text-center px-4">
                <div className="w-20 h-20 bg-gray-100 rounded-2xl flex items-center justify-center mb-6">
                    <FolderHeart className="text-gray-400" size={40} />
                </div>
                <h2 className="text-2xl font-bold mb-2">Login Required</h2>
                <p className="text-gray-500 mb-8 max-w-sm">
                    Please login to view and manage your collections
                </p>
                <button
                        ...
                        >
                            ...
                            <div className="flex items-center justify-between">
                                <div>
                                    <h3 className="font-bold text-lg group-hover:text-gray-600 transition-colors">{collection.title}</h3>
                                    <p className="text-sm text-gray-400 font-medium">
                                        {collection.photos?.length || 0} {collection.photos?.length === 1 ? 'photo' : 'photos'}
                                    </p>
                                </div>
                                <ArrowRight className="text-gray-300 group-hover:text-black group-hover:translate-x-1 transition-all" size={20} />
                            </div>
