...
export default function LikeButton({ 
    ...
    return (
        <button
            onClick={handleLike}
            disabled={loading}
            className={`flex items-center gap-2 transition-all duration-200 cursor-pointer group ${className} bg-white w-[40px] h-[32px] rounded-lg px-1.5 ${isLiked ? 'text-red-500' : 'text-gray-600 hover:text-red-400'} ${loading ? 'cursor-not-allowed opacity-50' : ''}`}
